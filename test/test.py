...
def test_equality(board_hash, board_matrix, times):
    for _ in range(times):
        board_from_server = [[colors[randint(0, 3)] for _ in range(9)] for _ in range(9)]
        board_hash.update(board_from_server)
        board_matrix.update(board_from_server)
        if sorted(board_hash.get_available_moves(('BLACK',))) != sorted(board_matrix.get_available_moves(('BLACK',))):
            return False
        elif sorted(board_hash.get_available_moves(('WHITE', 'KING'))) != sorted(board_matrix.get_available_moves(('WHITE', 'KING'))):
            return False
    return True
...
times = 10000
test = int(input("1: get_available_moves\n2: random_move\n3: update\n4: equality\n"))
if test == 1:
    ...
    print(test_equality(board_hash, board_matrix, times))
