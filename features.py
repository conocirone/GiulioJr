...
def king_safety(state, color):
    ...
    for i, j in state.color_coords["KING"]:
        king_position = (i, j)
        break
    ...
    close_blacks = 0
    if (
        state.coords_color.get((king_position[0] - 1, king_position[1]), None ) == "BLACK"
    ):  # Up square check
        close_blacks += 1
    if (
        state.coords_color.get((king_position[0] + 1, king_position[1]), None ) == "BLACK"
        ...
        close_blacks += 1
    if (
        state.coords_color.get((king_position[0], king_position[1] - 1), None ) == "BLACK"
    ):  # Left square check
        close_blacks += 1
    if (
        state.coords_color.get((king_position[0], king_position[1] + 1), None ) == "BLACK"
...
def capture_king(state, color):
    if len(state.color_coords["KING"]) == 0:
        if color == "WHITE":
            return float("-inf") 
        else:
            return float("inf")
...
def win_move_king(state, color):
    ...
    if king_position[0] in (2, 6):
        for col in range(9):
            if state.coords_color.get((king_position[0], col), None) is not None and (king_position[0], col) != (king_position[0], king_position[1]):
                found = True
                break
    else:
        for row in range(9):
            if state.coords_color.get((row, king_position[1]), None) is not None and (row, king_position[1]) != (king_position[0], king_position[1]):
                found = True
                break
        print('ciao')
        print('ciaociao')
